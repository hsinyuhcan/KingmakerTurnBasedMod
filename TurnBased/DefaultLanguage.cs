...
namespace TurnBased
{
    ...
    public class DefaultLanguage : ILanguage
    {
        ...
        public Dictionary<string, string> Strings { get; set; } = new Dictionary<string, string>()
        {
            ...
            { "Menu_Opt_FixAcrobaticsMobility", "Fix you can use Acrobatics (Mobility) to avoid AoO even if the AoO is provoked due to reasons other than movement" },
            { "Menu_Opt_FixCanMakeAttackOfOpportunityToUnmovedTarget", "Fix you can make an AoO to an unmoved unit as if it's leaving the threatened range (when switching from reach weapon)" },
            ...
            { "Menu_Opt_FixAbilityCanTargetDeadUnit", "Fix you can target dead units using abilities that cannot be cast on dead targets" },
            ...
            { "Menu_Opt_FixInspectingCauseError", "Fix inspecting enemies can cause errors in certain condition (e.g. when using Blindsense to against enemies who can ambush)" },
